...
export function CreateRewardConfigModal({ isOpen, onClose, config }: Props) {
  ...
  const [defaultLevelPercentageBps, setDefaultLevelPercentageBps] = useState("250");
  ...
  const addLevel = () => {    if (levels.length >= MAX_LEVELS) return;
    ...
    try {
      ...
      toast.error(
        apiError?.data?.message ??
          (isEditMode ? t("rewardConfigUpdateFailed") : t("rewardConfigCreateFailed"))
      );
    }
  };
