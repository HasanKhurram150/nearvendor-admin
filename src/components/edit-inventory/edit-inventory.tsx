...
const EditInventory: React.FC = () => {
  ...
  // const { data: inventoryDetail, isLoading: inventoryDetailLoading } =
  //   useGetInventoryByIdQuery(slug, { skip: !slug });
  ...
  const inventoryDetail: any = null;
  const inventoryDetailLoading = false;
  ...
  const handleLoadInventroryData = (inventory: IInventory) => {
    if (inventory) {
      setValue(
        "placementId",
        {
          label: inventory.placement.placementName,
          value: inventory.placement.id,
        },
        { shouldValidate: true },
      );
      setValue("materialType", inventory.placement.support, {
        shouldValidate: true,
      });
      setValue(
        "materialSize",
        `${inventory.placement.width}x${inventory.placement.height}`,
        {
          shouldValidate: true,
        },
      );
      ...
      setValue(
        "campaign",
        {
          label: inventory.campaign.name,
          value: inventory.campaign.id,
        },
        { shouldValidate: true },
      );
      setValue(
        ...
        inventory.campaign.budgetRemaining,
        { shouldValidate: true },
      );
      setValue("inventoryName", inventory.inventoryName, {
        shouldValidate: true,
      });
      setValue("inventoryDesc", inventory.inventoryDesc, {
        shouldValidate: true,
      });
      setValue("exposureStartDate", inventory.startDate, {
        shouldValidate: true,
      });
      setValue("exposureEndDate", inventory.endDate, {
        shouldValidate: true,
      });
      setValue(
        "impressionDay",
        inventory.impressionDay.split(",").map((dayNumber: string) => {
          return daysMap[dayNumber as DayNumber];
        }),
        {
          shouldValidate: true,
      ...
      setValue("dailyStartTime", `${inventory.startTime.split(".")[0]}`, {
        shouldValidate: true,
      });
      setValue("dailyEndTime", `${inventory.endTime.split(".")[0]}`, {
        shouldValidate: true,
      });
      setValue(
        "landingType",
        landingTypeOptions[inventory.landingType === "out_link" ? 1 : 0],
        {
          shouldValidate: true,
        },
      );
      setValue("landingUrl", inventory.landingUrl, {
        shouldValidate: true,
      });
      setValue("unitCost", inventory.unitCost, {
        shouldValidate: true,
      });
      setValue(
        "unitCostType",
        {
          label: inventory.costType,
          value: inventory.costType,
        },
        {
          shouldValidate: true,
        },
      );
      setValue("creativeFile", inventory.file1, {
        shouldValidate: true,
      });
    }
  };
  ...
  useEffect(() => {
    ...
    handleLoadInventroryData(inventoryDetail);
  }, [inventoryDetail]);

  return (
    <>
      <PageBreadcrumb pageTitle={t("updateInventory")} />
              ...
              <Controller
                ...
                render={() => (
                  <SearchableDropdown
                    ...
                    isLoading={isFetchingPlacements || inventoryDetailLoading}
              ...
              <Controller
                ...
                render={() => (
                  <SearchableDropdown
                    ...
                    isLoading={isFetchingCampaigns || inventoryDetailLoading}
              ...
              <Controller
                ...
                render={() => (
                  <SearchableDropdown
                    ...
                    isLoading={inventoryLoading || inventoryDetailLoading}
